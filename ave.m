...
%     vec2=squeeze(wfall(kindex,2,:));
    ave=ave+(vec(:,1)'*mat*vec(:,1)*fd(kindex,1)*debye(kindex,1)...
                +vec(:,2)'*mat*vec(:,2)*fd(kindex,2)*debye(kindex,2)...
                +vec(:,3)'*mat*vec(:,3)*fd(kindex,3)*debye(kindex,3)...
                +vec(:,4)'*mat*vec(:,4)*fd(kindex,4)*debye(kindex,4));
