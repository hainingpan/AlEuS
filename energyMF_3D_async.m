function [energyall,wfall]=energyMF_3D_async(ave,wfall,param)
%wfall(kz,NxNy,:)
tic;
sz=param.sz;
sy=param.sy;
...
for idx = 1:length(knzlist)
    f(idx) = parfeval(@(x)diagonize(x,wfall,pp),3,knzlist(idx));
end
...
function [val,vec,string]=diagonize(kindex,wfall,pp)
